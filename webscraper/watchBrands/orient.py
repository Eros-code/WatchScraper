...
def orient_watches(orient_url):
    ...
    for i in range(0, int(max_pages(orient_url))):
        ...
        for link in orient.find_all("a", {"class": "product-card__link"}):
            ...
            type = link.find("h3", {"class": "product-card__name"}).text.replace('\n', '').strip()
            url = orient_base_url + link['href']
            orient_watches[model] = {'type':type, 'price': price, 'url':url, 'image':orient_image[count]}
            count += 1
    return orient_watches

print(orient_watches(orient_url))